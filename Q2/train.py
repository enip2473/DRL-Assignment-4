...
def evaluate_policy(env, agent, episodes=5):
    ...
    for _ in range(episodes):
        state, _ = env.reset(seed=random.randint(0, 1000000)) # Use different seeds for eval episodes
        episode_reward = 0.0
        terminated = False
        truncated = False
...
if __name__ == "__main__":
    ...
    eval_env = make_dmc_env(ENV_NAME, np.random.randint(0, 1000000), flatten=True, use_pixels=False)
